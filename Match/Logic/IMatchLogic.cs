...
namespace OpenGSServer
{
    ...
    // ワンショットキルモード
    public class OneShotKillMode : IMatchLogic
    {
        private Dictionary<string, int> playerScores = new();
        private List<string> players = new();
        private bool matchActive = false;
        private const int WIN_KILL_COUNT = 10;

        public void StartMatch()
        {
            matchActive = true;
            Console.WriteLine("One Shot Kill Mode started");
        }

        public void EndMatch()
        {
            matchActive = false;
            Console.WriteLine("One Shot Kill Mode ended");
        }
        ...
        public string GetMatchStatus()
        {
            ...
            var topPlayer = playerScores.OrderByDescending(x => x.Value).FirstOrDefault();
            return $"One Shot Kill Active - Leader: {topPlayer.Key} ({topPlayer.Value} kills)";
        }
    ...
    // アームズレースモード
    public class ArmsRaceMode : IMatchLogic
    {
        private Dictionary<string, int> playerLevels = new();
        private List<string> players = new();
        private bool matchActive = false;
        ...
        {
            matchActive = true;
            Console.WriteLine("Arms Race Mode started");
        }

        public void EndMatch()
        {
            matchActive = false;
            Console.WriteLine("Arms Race Mode ended");
        }
        ...
        public string GetMatchStatus()
        {
            ...
            var topPlayer = playerLevels.OrderByDescending(x => x.Value).FirstOrDefault();
            return $"Arms Race Active - Leader: {topPlayer.Key} (Level {topPlayer.Value})";
        }
        ...
        public void UpdateScore(string playerName, int score)
        {
            ...
            {
                playerLevels[playerName] = Math.Min(MAX_LEVEL, playerLevels[playerName] + score);
            }
        }
        ...
        public bool CheckWinCondition()
        {
            ...
            return playerLevels.Any(x => x.Value >= MAX_LEVEL);
        }

        public List<string> GetWinners()
        {
            var maxLevel = playerLevels.Max(x => x.Value);
            return playerLevels.Where(x => x.Value == maxLevel).Select(x => x.Key).ToList();
        }
    }
}
